    ...
    with DBConnection(db_creds()).conn as conn:
            try:
                with conn.cursor() as curs:
                    curs.execute("SELECT count(*) FROM citybikes.edl WHERE messagesent='networks' AND NOT processed;")
                    networks_updates_count = curs.fetchone()[0]
            except Exception as e:
                print(logging.error(traceback.format_exc()))

    print("[load.networks_to_edw()] ",networks_updates_count," updates found...")
...
def station_to_edw():
    ...
    with DBConnection(db_creds()).conn as conn:
            try:
                with conn.cursor() as curs:
                    curs.execute("SELECT count(*) FROM citybikes.edl WHERE messagesent='fortworth' AND NOT processed limit 500;")
                    station_updates_count = curs.fetchone()[0]
            except Exception as e:
                print(logging.error(traceback.format_exc()))

    print("[load.stations_to_edw()] ",station_updates_count," updates found...")
