...
def stations_in_edw():
    ...
    try:
        with DBConnection(db_creds()).conn as conn:
            df_stations = fetch_db_data(
                conn,
                'citybikes.stations',
                columns,
                filter="timestamp_custom_bin_hour is null",
            )
    except Exception as e:
        logging.error(traceback.format_exc())
    print("YO")
    print(
        f"[transform.stations_in_edw()] df_stations.count = {df_stations.count}"
    )

    # Add bin for timestamp
    tz_offset = 5
    df_stations['timestamp_custom_bin_hour'] = (
        df_stations['timestamp'].dt.hour - tz_offset
    ) % 24

    print(
        "[transform.stations_in_edw()] df_stations.'timestamp_custom_bin_hour' = \n",
...
def networks_in_edw():
    ...
    print(
        f"[transform.networks_in_edw()] df_networks.count = {df_networks.count}"
    )
